...
def load_knowhy(url: str, html: str, bs_parser: str = "html.parser") -> Document:
    ...
    title = get_text(soup.find("h1", class_="page-title"))
    author = get_text(soup.find("div", class_="field-nam-author")).replace("Post contributed by", "")
    date = get_text(soup.find("div", class_="field-name-publish-date"))
